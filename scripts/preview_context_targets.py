...
def build_flow_step_views(
    ...
    out_ctx = float(
        sample_metadata.get(
            "effective_context_mpp",
            sample_metadata.get("output_context_mpp", sample_metadata.get("requested_context_mpp")),
        )
    )
    out_tgt = float(
        sample_metadata.get(
            "effective_target_mpp",
            sample_metadata.get("output_target_mpp", sample_metadata.get("requested_target_mpp")),
        )
    )
    right_views = [
        context_with_box,
...
def main() -> int:
    ...
    for i in range(limit):
        ...
        thumbnail_rgb, anchor_box_thumb = _thumbnail_from_wsi_row(
            ...
            context_mpp=float(
                sample_metadata.get("effective_context_mpp", sample_metadata.get("output_context_mpp", context_mpp))
            ),
            context_fov_um=context_fov_um,
        )
