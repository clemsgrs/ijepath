#!/usr/bin/env python3
"""Download PathoROB CAMELYON from Hugging Face and write ijepath manifest."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Download CAMELYON PathoROB dataset and build manifest."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/pathorob/camelyon"),
        help="Destination folder for images/manifest.",
    )
    parser.add_argument(
        "--repo-id",
        type=str,
        default="bifold-pathomics/PathoROB-camelyon",
    ...
    parser.add_argument(
        "--label-column",
        type=str,
        default="biological_class",
        help="Column containing biological label.",
    )
    parser.add_argument(
        "--center-column",
        type=str,
        default="medical_center",
        help="Column containing medical center.",
    )
    parser.add_argument(
        "--slide-id-column",
        type=str,
...
def main() -> int:
    ...
    for idx, sample in enumerate(dataset):
        ...
        records.append(
            {
                "sample_id": f"camelyon_{idx:09d}",
                "image_path": str(image_path.resolve()),
                "label": str(sample[args.label_column]),
                "medical_center": str(sample[args.center_column]),
    ...
    print("Manifest columns: image_path,label,medical_center,slide_id,sample_id")
    return 0
