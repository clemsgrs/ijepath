...
def flatten_teacher_tokens_for_predictor_order(
    ...
    `teacher` shape is [B*num_pred_masks, N, D].
    Predictor output order is pred-mask-major, and repeated per encoder mask.
    """
    if teacher.dim() != 3:
        raise ValueError("teacher must be rank-3 [B*num_pred_masks, N, D]")
    if teacher.shape[0] != int(batch_size) * int(num_pred_masks):
        raise ValueError("teacher token batch dimension mismatch")
    if num_enc_masks > 1:
...
def validate_encoder_input_size(
    *,
    images: torch.Tensor,
    expected_input_size_px: int,
    patch_size: int,
) -> None:
    if images.ndim != 4:
        raise ValueError(
            "images must be rank-4 [B, C, H, W], "
            f"got shape={tuple(images.shape)}"
        )

    height = int(images.shape[-2])
    width = int(images.shape[-1])
    expected = int(expected_input_size_px)
    patch = int(patch_size)

    if height != expected or width != expected:
        raise ValueError(
            "Image size mismatch: "
            f"expected={expected}x{expected} got={height}x{width}"
        )
    if expected % patch != 0:
        raise ValueError(
            "Expected input size must be divisible by patch size: "
            f"input={expected} patch={patch}"
        )
    if height % patch != 0 or width % patch != 0:
        raise ValueError(
            "Batch tensor shape is not divisible by patch size: "
            f"input={height}x{width} patch={patch}"
        )
...
def main(
    ...
    # -- CANONICAL
    canonical_cfg = dict(args.get("canonical", {}) or {})
    ...
    args["data"]["canonical_input_size_px"] = int(model_input_size_px)
    ...
    unsupervised_dataset, unsupervised_loader, unsupervised_sampler = make_canonical_loader(
    ...
    if float(anchor_budget["anchor_passes_total"]) < float(low_anchor_pass_warning_threshold):
        logger.warning(
            ...
            "For extra mask-target diversity per anchor, increase canonical.num_pred_masks."
        )
        ...
        while steps_done < total_steps and not should_stop_training:
            ...
            with tqdm.tqdm(
                ...
                for itr in range(ipe):
                    ...
                    validate_encoder_input_size(
                        images=images,
                        expected_input_size_px=model_input_size_px,
                        patch_size=patch_size,
                    )
                    ...
                    def train_step():
                        ...
                        def forward_target():
                            with torch.no_grad():
                                ...
                                h = flatten_teacher_tokens_for_predictor_order(
                                    teacher=h,
