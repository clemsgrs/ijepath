...
def test_robustness_early_stopper_tracks_best_and_stops(tmp_path: Path):
    ...
    snapshot = {"x": 1}
    stopper.on_tune(metric_value=0.1, snapshot=snapshot)
    stopper.on_tune(metric_value=0.09, snapshot=snapshot)
    assert stopper.should_stop is False
    stopper.on_tune(metric_value=0.08, snapshot=snapshot)
...
def test_robustness_early_stopper_saves_latest_best_metric(tmp_path: Path):
    ...
    stopper.on_tune(metric_value=3.0, snapshot=snapshot_a)
    stopper.on_tune(metric_value=1.0, snapshot=snapshot_b)
...
def test_robustness_early_stopper_can_skip_checkpoint_saving(tmp_path: Path):
    ...
    stopper.on_tune(metric_value=0.3, snapshot={"metric": 0.3})
